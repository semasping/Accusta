...
class WhalesharesApi
{
    ...
    public static function getConnector()
    {
        if (self::$connector == null) {
            self::$connector = new WhalesharesApiHttpJsonRpcConnector();
        }
    ...
    {
        $key = "1whaleshares_getacchistory.$acc.$from";
            ...
            if ($from % $limit == 0) {
                ...
                $history = Cache::rememberForever($key,
                    function () use ($acc, $from, $limit) {
                        //AdminNotify::send("whaleshares to set cache getHistoryAccount($acc, $from, $limit) in function");
    ...
    public static function getHistoryAccountLast($acc)
    {
        ...
        //dump($res);
        //AdminNotify::send("whaleshares: max = getHistoryAccountLast($acc) = " . $res['result'][0][0]);
...
//dump($res);
    ...
    public static function getHistoryAccountAll($acc)
    {
        ...
        return Cache::rememberForever('whaleshares_resulthistory' . $acc . $max,
            function () use ($max, $acc) {
                ...
                while ($i <= $max) {
                    ...
                    $qq++;
                    if ($qq > 5000) {
                        AdminNotify::send('$qq > 5000 in WhalesharesApi.php:145');
                        break;
                    }
                }
...
//@todo тут вот забивает диск лишними кешами. Надо переписать чтобы удаляло старые кеши которые уже ненужны
        $return = false;
        $key = "1whaleshares_getfullacchis.$acc.$max";
        $key2 = "1whaleshares_getfullacchis.$acc";
            ...
            $return = Cache::remember($key2 . $max, 100,
                function () use ($max, $acc, $key2) {
                    ...
                    while ($i <= $max) {
                        ...
                        $qq++;
                        if ($qq > 5000) {
                            AdminNotify::send('$qq > 5000 in WhalesharesApi.php:188');
                            break;
                        }
                    }
...
//            $command = new GetContentCommand(self::getConnector());
    ...
    {
        return Cache::remember('whaleshares_getPrice_', 10, function () {
            $resp = self::GetDynamicGlobalProperties();
            //AdminNotify::send(print_r($resp,true));
            if (is_array($resp)) {
                $q1 = str_replace(' WLS', '', $resp['total_vesting_fund_steem']);
...
//dump(($history));
    ...
    {
        return '1whaleshares_current_cache_transactions_' . $acc;
    }
    ...
    private static function prepare_transactions($transaction)
    {
        ...
        if ($trns['op'][0] == 'claim_reward_balance') {
            $trns['op'][1]['STEEM'] = (double)((str_replace(' WLS', '',
